...
(function() {
    ...
    async function loadProvincias() {
        ...
        if (municipioStatus) {
            municipioStatus.textContent = 'Cargando comunidades...';
        }
        if (municipioLoading) {
            municipioLoading.style.display = 'inline-block';
        }
        setStatus('Cargando comunidades desde la API...', 'loading');
        ...
        try {
            ...
            if (response.data.success && response.data.data) {
                ...
                // Limpiar select
                municipioSelect.innerHTML = '<option value="">Selecciona una comunidad autónoma...</option>';
                
                // ✅ Añadir opciones con ID encriptado (ya viene encriptado del backend)
                if (municipios.length === 0) {
                    municipioSelect.innerHTML = '<option value="">No hay comunidades autónomas disponibles</option>';
                    if (municipioStatus) {
                        municipioStatus.textContent = '⚠️ No hay comunidades autónomas disponibles';
                    }
                    setStatus('No hay comunidades autónomas disponibles', 'warning');
                ...
                municipiosCargados = true;
                if (municipioStatus) {
                    municipioStatus.textContent = `✅ ${municipios.length} comunidades autónomas cargadas`;
                }
                setStatus(`${municipios.length} comunidades autónomas disponibles`, 'success');
                ...
                throw new Error(response.data.error || 'Error al cargar comunidades autónomas');
            }

        } catch (error) {
            console.error('Error cargando comunidades autónomas:', error);
            
            let errorMsg = 'Error al cargar comunidades autónomas';
            ...
            municipioSelect.innerHTML = `<option value="">Error al cargar comunidades autónomas</option>`;
        }
    ...
    async function consultarFestivos() {
        ...
        if (!encryptedMunicipioId) {
            setStatus('⚠️ Selecciona una comunidad autónoma', 'warning');
        ...
        try {
            ...
            let Direccion_API=API_BASE_URL+BASE_URL+'/'+API_BASE+'/festivos-comunidad-autonoma';
            const response = await axios.get(Direccion_API, {
                params: {
                    comunidad_id: encryptedMunicipioId,  // 🔒 ID encriptado
                    anio: anio
                },
                headers: {
                    'X-API-Key': API_KEY
